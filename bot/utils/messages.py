...
def start_text() -> str:
    return (
        ...
        "Что умею:\n"
        "- показывать статистику и депозит\n"
        "- показывать последние сделки и историю\n"
        "- запускать пересчет статистики\n"
        "- проверять соблюдение риск-правил\n"
        "- показывать недельный обзор\n\n"
        "Чего не умею:\n"
        "- не торгую\n"
        "- не подключаюсь к бирже\n"
        "- не даю гарантий доходности\n\n"
        "Как начать: используйте кнопки ниже или команды /help и /stats."
    )
...
def screenshot_stub_text() -> str:
    return "Раздел 'Скрин сделки' в v1 пока в разработке."
