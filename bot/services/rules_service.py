...
def evaluate_rules(project_root: Path) -> list[str]:
    ...
    if not rules:
        return ["Файл config/risk_rules.json не найден или поврежден."]
    ...
    for row in trades:
        ...
        for never_rule in rules.get("never_rules", []):
            ...
            if rule_text and rule_text in notes_blob:
                violations.append(f"{trade_id}: возможное нарушение '{never_rule}'")
    ...
    if max_trades_per_day is not None:
        for day, count in trades_per_day.items():
            if count > int(max_trades_per_day):
                violations.append(f"{day}: превышен лимит сделок за день ({count} > {max_trades_per_day})")
    ...
    if max_daily_loss is not None:
        for day, pnl in pnl_per_day.items():
            if pnl < 0 and abs(pnl) > float(max_daily_loss):
                violations.append(
                    f"{day}: превышен дневной лимит убытка ({abs(pnl):.2f} > {float(max_daily_loss):.2f})"
                )
